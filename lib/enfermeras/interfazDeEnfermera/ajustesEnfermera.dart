...
class AjustesEnfermera extends StatefulWidget {
  @override
  _AjustesEnfermeraState createState() => _AjustesEnfermeraState();
}
...
class _AjustesEnfermeraState extends State<AjustesEnfermera> {
  ...
  void _getCurrentUserData() async {
    ...
    setState(() {
      ...
      final categoria = userData.get('categoria');
  ...
  Future<void> _uploadAndSetPhoto() async {
    ...
    if (pickedFile != null) {
      ...
      final destination =
          'usuarios/enfermeras/${FirebaseAuth.instance.currentUser!.uid}/photoUrl';
  ...
  Future<void> _updateUserStateAndMunicipality(
    ...
    try {
      final userId = FirebaseAuth.instance.currentUser!.uid;
      final userRef =
          FirebaseFirestore.instance.collection('usuarioenfermera').doc(userId);
  ...
  void _updateDistance(double newDistance) async {
    final userId = FirebaseAuth.instance.currentUser!.uid;
    await FirebaseFirestore.instance
        .collection('usuarioenfermera')
        .doc(userId)
        .update({'distancia': newDistance});

    setState(() {
      _userDistance = newDistance;
    });
  }
          ...
          children: [
            ...
            Center(
              child: Column(
                children: [
                  Text(
                    'Categoría: ${_categoria == "Basico" ? "Básico" : _categoria == "Intermedio" ? "Intermedio" : _categoria == "Avanzado" ? "Avanzado" : _categoria}',
                    style: TextStyle(
                      fontSize: 15,
                      fontWeight: FontWeight.bold,
                      color: Color(0xFF245366),
            ...
            Padding(
              padding: const EdgeInsets.symmetric(
                  vertical: 0), // Agrega un padding vertical
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Expanded(
                    child: Text(
                      'Distancia: $_userDistance Km',
                      style: TextStyle(
                        fontSize: 16,
                        color: Color(0xFF245366),
                      ),
                  ...
                  Container(
                    width: 226, // Cambia este valor al ancho deseado
                    child: Slider(
                      value: _userDistance,
                      min: 0,
                      max: 100,
                      ...
                      label: _userDistance.round().toString(),
            ...
            Text(
              'Ajusta la distancia máxima para recibir solicitudes dentro de tu zona (al actualizar, reinicia la aplicación para recibir servicios con el nuevo rango).',
              style: TextStyle(
                fontSize: 14,
                color: Colors.grey,
              ),
